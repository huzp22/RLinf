...
本脚本来自 OpenPI ``examples/convert_jax_model_to_pytorch.py``，放在 RLinf 的
``toolkits/openpi`` 下，供本地 Orbax ``params/`` 目录一键转为 ``model.safetensors``，
供 ``FSDPVlaSftWorker`` 等 PyTorch 加载路径使用。

Usage:
    # 仅查看参数键:
    python toolkits/openpi/convert_jax_model_to_pytorch.py --checkpoint-dir /path/to/pi0_base --inspect-only

    # 转换（默认 ``--config-name pi0_libero`` 对应标准 π₀ 基座 ``Pi0Config()``）:
    python toolkits/openpi/convert_jax_model_to_pytorch.py --checkpoint-dir /path/to/pi0_base \\
        --output-path /path/to/pi0_base_pytorch

Example:
    python toolkits/openpi/convert_jax_model_to_pytorch.py \\
        --checkpoint-dir /mnt/public/datasets/pretrained-checkpoints/openpi-assets/checkpoints/pi0_base \\
        --output-path /mnt/public/datasets/pretrained-checkpoints/openpi-assets/checkpoints/pi0_base_pytorch
"""
...
# 从 RLinf 仓库运行时，将同级目录下的 openpi/src 加入 path（与已安装 openpi 二选一即可）。
_rlinf_repo = pathlib.Path(__file__).resolve().parents[2]
_openpi_src = _rlinf_repo.parent / "openpi" / "src"
if _openpi_src.is_dir():
    _openpi_src_s = str(_openpi_src)
    if _openpi_src_s not in sys.path:
        sys.path.insert(0, _openpi_src_s)
...
def convert_pi0_checkpoint(
    ...
    # Copy assets folder if it exists (标准布局为 <checkpoint_dir>/assets)
    ckpt = pathlib.Path(checkpoint_dir)
    assets_source = ckpt / "assets"
    if not assets_source.exists():
        alt = ckpt.parent / "assets"
        if alt.exists():
            assets_source = alt
...
def main(
    ...
    *,
    ...
    Args:
        checkpoint_dir: Path to the JAX checkpoint directory (含 ``params/`` 子目录)
        config_name: OpenPI ``TrainConfig`` 注册名，其 ``model`` 须为 ``Pi0Config``；默认 ``pi0_libero`` 对应 π₀ 基座结构
